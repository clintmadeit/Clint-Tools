...
void v_default_mix(){
    int f_i;
    int framesPerBuffer = clinttools->sample_count;
    float* out = CLINTTOOLS->out;
...
void g_clinttools_get(
    SGFLT a_sr,
    t_midi_device_list* a_midi_devices
){
    clalloc((void**)&CLINTTOOLS, sizeof(t_clinttools));
    pthread_mutex_init(&CLINTTOOLS->audio_inputs_mutex, NULL);
    CLINTTOOLS->audio_pool = g_audio_pool_get(a_sr);
    CLINTTOOLS->midi_devices = a_midi_devices;
    CLINTTOOLS->current_host = NULL;
    CLINTTOOLS->sample_count = 512;
    CLINTTOOLS->midi_learn = 0;
    CLINTTOOLS->is_offline_rendering = 0;
    pthread_spin_init(&CLINTTOOLS->main_lock, 0);

    CLINTTOOLS->preview_wav_item = 0;
    CLINTTOOLS->preview_audio_item = g_audio_item_get(a_sr);
    CLINTTOOLS->preview_start = 0.0f;
    CLINTTOOLS->preview_amp_lin = 1.0f;
    CLINTTOOLS->is_previewing = 0;
    CLINTTOOLS->preview_max_sample_count = ((int)(a_sr)) * 30;
    CLINTTOOLS->playback_mode = 0;

    int f_i;

    hpalloc(
        (void**)&CLINTTOOLS->audio_inputs,
        sizeof(t_audio_input) * AUDIO_INPUT_TRACK_COUNT
    );

    for(f_i = 0; f_i < AUDIO_INPUT_TRACK_COUNT; ++f_i){
        g_audio_input_init(
            &CLINTTOOLS->audio_inputs[f_i],
            a_sr
        );
    }
    ...
    for(f_i = 0; f_i < MAX_WORKER_THREADS; ++f_i){
        ts = &CLINTTOOLS->thread_storage[f_i];
        ts->sample_rate = a_sr;
    ...
    pthread_spin_init(&CLINTTOOLS->ui_spinlock, 0);
    CLINTTOOLS->osc_queue_index = 0;

    for(f_i = 0; f_i < MAX_PLUGIN_POOL_COUNT; ++f_i){
        CLINTTOOLS->plugin_pool[f_i].active = 0;
    }
}
...
void v_set_host(int a_mode){
    ...
    CLINTTOOLS->current_host = &CLINTTOOLS->hosts[a_mode];

    for(f_i = 0; f_i < MAX_WORKER_THREADS; ++f_i){
        CLINTTOOLS->thread_storage[f_i].current_host = a_mode;
    }
    ...
    if(CLINTTOOLS->midi_devices){
        for(f_i = 0; f_i < CLINTTOOLS->midi_devices->count; ++f_i){
            f_device = &CLINTTOOLS->midi_devices->devices[f_i];
            if(f_device->loaded){
                midiPoll(f_device);
    ...
    if(CLINTTOOLS->current_host->audio_inputs){
        CLINTTOOLS->current_host->audio_inputs();
    }
}
    ...
    if(i_file_exists(f_file_name)){
        ...
        while(1){
            ...
            if(f_2d_array->current_str[0] == 'p'){
                ...
                int audio_input = 1;
                int midi_channel = 0;
                // TODO: clinttools v2: Remove if statements
                if(!f_2d_array->eol){
                    v_iterate_2d_char_array(f_2d_array);
...
void v_set_preview_file(const char* a_file){
    ...
    if(f_result){
        if(i_audio_pool_item_load(f_result, 0)){
            t_audio_pool_item * f_old = CLINTTOOLS->preview_wav_item;

            pthread_spin_lock(&CLINTTOOLS->main_lock);

            CLINTTOOLS->preview_wav_item = f_result;

            CLINTTOOLS->preview_audio_item->ratio =
                    CLINTTOOLS->preview_wav_item->ratio_orig;

            CLINTTOOLS->is_previewing = 1;

            v_ifh_retrigger(
                &CLINTTOOLS->preview_audio_item->sample_read_heads[0],
                AUDIO_ITEM_PADDING_DIV2
            );
            v_adsr_retrigger(
                &CLINTTOOLS->preview_audio_item->adsrs[0]
            );
...
void stop_preview(){
    if(CLINTTOOLS->is_previewing){
        pthread_spin_lock(&CLINTTOOLS->main_lock);
        v_adsr_release(&CLINTTOOLS->preview_audio_item->adsrs[0]);
        pthread_spin_unlock(&CLINTTOOLS->main_lock);
    }
}
...
NO_OPTIMIZATION void v_wait_for_threads(){
    ...
    for(i = 1; i < (CLINTTOOLS->worker_thread_count); ++i){
        lock = &CLINTTOOLS->worker_threads[i].lock;
    ...
    for(
        ...
        self->atm_ticks[self->atm_tick_count].beat =
            ...
            f_samples_to_beat_count(
                ...
                a_event_list->tempo,
                CLINTTOOLS->thread_storage[0].sr_recip
            );
...
void v_sg_configure(const char* a_key, const char* a_value){
    ...
    if(!strcmp(a_key, SG_CONFIGURE_KEY_UPDATE_PLUGIN_CONTROL)){
        ...
        f_instance = &CLINTTOOLS->plugin_pool[f_plugin_uid];
        ...
        t_plugin * f_instance = &CLINTTOOLS->plugin_pool[f_plugin_uid];
        ...
        t_audio_input * f_input = &CLINTTOOLS->audio_inputs[f_index];
        f_input->vol = f_vol;
        ...
        int f_plugin_uid = atoi(f_val_arr->array[0]);
        CLINTTOOLS->plugin_pool[f_plugin_uid].descriptor->set_cc_map(
            CLINTTOOLS->plugin_pool[f_plugin_uid].plugin_handle,
            f_val_arr->array[1]
        );
        g_free_1d_char_array(f_val_arr);
    } else if(!strcmp(a_key, SG_CONFIGURE_KEY_MIDI_LEARN)){
        CLINTTOOLS->midi_learn = 1;
	...
	utf8_to_utf16(
        ...
        pthread_spin_lock(&CLINTTOOLS->main_lock);
        CLINTTOOLS->audio_pool->items[uid].volume = vol;
        pthread_spin_unlock(&CLINTTOOLS->main_lock);
    ...
    else if(!strcmp(a_key, SG_CONFIGURE_KEY_ENGINE))
    {
        ...
        pthread_spin_lock(&CLINTTOOLS->main_lock);
        CLINTTOOLS->is_offline_rendering = f_val;
        pthread_spin_unlock(&CLINTTOOLS->main_lock);
    }
    else if(!strcmp(a_key, SG_CONFIGURE_KEY_PITCH_ENV))
    {
        ...
        while(!f_arr->eof){
            ...
            v_audio_pool_remove_item(CLINTTOOLS->audio_pool, f_uid);
        }
        ...
        t_audio_pool_item f_new = {
            .uid = f_uid,
            .host_sr = CLINTTOOLS->audio_pool->sample_rate,
        };
#if SG_OS == _OS_WINDOWS
...
t_audio_pool_item* g_audio_pool_item_get_plugin(int a_uid){
    return g_audio_pool_get_item_by_uid(CLINTTOOLS->audio_pool, a_uid);
}
...
void plugin_init(
    ...
    int sample_rate = (int)CLINTTOOLS->thread_storage[0].sample_rate;
...
NO_OPTIMIZATION void v_set_plugin_index(
    ...
    if(a_plugin_index){
        ...
        f_plugin = &CLINTTOOLS->plugin_pool[a_plugin_uid];
        plugin_init(
            f_plugin,
            a_plugin_index,
            a_plugin_uid
...
void v_run(
    ...
    if(likely(!CLINTTOOLS->is_offline_rendering)){
...
void preview_sample(
    ...
    int f_i;
    t_audio_item * f_audio_item = CLINTTOOLS->preview_audio_item;
    t_audio_pool_item * f_wav_item = CLINTTOOLS->preview_wav_item;
    for(f_i = 0; f_i < sample_count; ++f_i){
        if(
            ...
            if(
                f_audio_item->sample_read_heads[0].whole_number
                >= 
                CLINTTOOLS->preview_max_sample_count
            ){
...
void v_run_main_loop(
    ...
    CLINTTOOLS->current_host->run(sample_count, a_buffers, a_input_buffers);

    if(unlikely(CLINTTOOLS->is_previewing)){
        preview_sample(sample_count, a_buffers);
    }

    if(!CLINTTOOLS->is_offline_rendering && MAIN_VOL != 1.0f){
        int f_i;
        for(f_i = 0; f_i < sample_count; ++f_i){
